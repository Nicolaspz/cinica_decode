    ...
    public function store(Request $request){
        ...
        for ($i=0; $i<7 ; $i++) {
            if($morning_start[$i]> $morning_end[$i]){
                if($i>4){
                    $errors[]='Incopatiblidade no intervalo da Hora do '. $days[$i] . ' no turno da Manhã';
                } else {
                    $errors[]='Incopatiblidade no intervalo da Hora da ' . $days[$i] . ' Feira no turno da Manhã ';
                }

            }
            if($afternoon_start[$i]> $afternoon_end[$i]){
                if($i>4){
                    $errors[]='Incopatiblidade no intervalo da Hora do '. $days[$i] . ' no turno da Tarde';
                } else {
                    $errors[]='Incopatiblidade no intervalo da Hora da ' . $days[$i] . ' Feira no turno da Tarde';
                }
            }
       ...
       $notifica="Horas salvas com suvesso";
       return back()->with(compact('notifica'));
